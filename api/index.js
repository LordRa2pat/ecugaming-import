...
app.get('/api/config', (req, res) => {
    res.json({
        ...
        supabaseAnonKey: process.env.SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRwb21rY2h2anBka25ka2tzcGh5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI0OTQ4OTgsImV4cCI6MjA4ODA3MDg5OH0.tqjYkOI9O42Bh2pA83yRiPn7_Q457B9-VcwWeL9A_rM'
    });
});
...
app.post('/api/credit-check', async (req, res) => {
    ...
    try {
        ...
        const dbPayload = {
            status: 'pendiente',
            // Personal
            nombres: sanitize(sol.nombres || sol.nombre_completo || ''),
            ...
            empleador: sanitize(sol.lugar_trabajo || eco.lugar_trabajo || ''),
            lugar_trabajo: sanitize(sol.lugar_trabajo || eco.lugar_trabajo || ''),
        ...
        if (useSupabase) {
            ...
            console.warn('[credit-check] Supabase not active, application not saved to database.');
        }
